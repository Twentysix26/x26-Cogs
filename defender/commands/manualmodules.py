...
class ManualModules(MixinMeta, metaclass=CompositeMetaClass):  # type: ignore
    ...
    async def alert(self, ctx):
        ...
        await ctx.send(
            "The staff has been notified. Please keep calm, I'm sure everything is fine. 🔥"
        )
        ...
        if (
            minutes
        ):  # This whole countdown thing is skipped if the max inactivity is a single minute
            text = (
            ...
            while minutes != 0:
                ...
                if self.has_staff_been_active(guild, minutes=1):
                    ...
                    ctx.command.reset_cooldown(ctx)
                    await ctx.send(
                        "Staff activity detected. Alert deactivated. "
        ...
        guide = {
            ...
            EmergencyModules.Vaporize: (
                "vaporize <users...>` - Allows you to mass ban users from " "the server"
            ),
            EmergencyModules.Silence: (
                "silence <rank> (2-4)` - Enables auto-deletion of messages for "
    ...
    async def vaporize(self, ctx, *members: discord.Member):
        ...
        if not override:
            if is_helper:
                ...
                if em_enabled:
                    return await ctx.send(
                        "This command is only available during emergency mode. "
            ...
            if is_staff and not has_ban_perms:
                ...
                if em_enabled:
                    return await ctx.send(
                        "You need ban permissions to use this module outside of emergency mode."
                    )
                else:
        ...
        for m in members:
            ...
            if rank < Rank.Rank3:
                await ctx.send(
                    "This command can only be used on Rank 3 and under. "
        ...
        for m in members:
            try:
                await guild.ban(
                    m, reason=f"Vaporized by {ctx.author} ({ctx.author.id})", delete_message_days=0
                )
            except:
        ...
        await self.send_notification(
            guild,
            f"{total} users have been vaporized.",
            title=EMBED_TITLE,
            fields=EMBED_FIELDS,
            jump_to=ctx.message,
        )
    ...
    @commands.cooldown(
        1, 22, commands.BucketType.guild
    )  # More useful as a lock of sorts in this case
    ...
    async def voteout(self, ctx, *, user: discord.Member):
        ...
        if not override:
            ...
            if is_staff and not has_action_perms:
                ...
                if em_enabled:
                    return await ctx.send(
                        f"You need {perm_text} permissions to use this module outside of "
                        "emergency mode."
                    )
                else:
        ...
        if Action(action) == Action.Ban:
            ...
            if punish_role and not self.is_role_privileged(punish_role):
                ...
                self.send_to_monitor(
                    guild,
                    "[Voteout] Failed to punish user. Is the punish role "
                    "still present and with *no* privileges?",
                )
                await ctx.channel.send(
                    "The voting session passed but I was not able to punish the "
    ...
    async def silence(self, ctx: commands.Context, rank: int):
        ...
        if not override:
            ...
            if is_staff and not has_mm_perms:
                ...
                if em_enabled:
                    return await ctx.send(
                        "You need manage messages permissions to use this "
                        "module outside of emergency mode."
                    )
                else:
                    return await ctx.send(
                        "You need manage messages permissions to use this module."
                    )
        ...
        if rank:
            ...
            await ctx.send(
                f"Any message from Rank {rank} and below will be deleted. "
