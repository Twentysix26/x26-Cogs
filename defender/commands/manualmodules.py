...
class ManualModules(MixinMeta, metaclass=CompositeMetaClass):  # type: ignore
    ...
    async def alert(self, ctx):
        ...
        if minutes:  # This whole countdown thing is skipped if the max inactivity is a single minute
            text = (
                "⚠️ No staff activity detected in the past minute. "
            ...
            await self.send_notification(
                guild,
                "⚠️ Looks like you're not around. I will automatically engage "
            ...
            while minutes != 0:
                ...
                if self.has_staff_been_active(guild, minutes=1):
                    ...
                    ctx.command.reset_cooldown(ctx)
                    await ctx.send(
                        "Staff activity detected. Alert deactivated. " "Thanks for helping keep the community safe."
                    )
                    return
                minutes -= 1
        ...
        guide = {
            ...
            EmergencyModules.Vaporize: ("vaporize <users...>` - Allows you to mass ban users from " "the server"),
            EmergencyModules.Silence: (
                "silence <rank> (2-4)` - Enables auto-deletion of messages for " "the specified rank (and below)"
            ),
        }

        text = (
            "⚠️ Emergency mode engaged. Helpers, you are now authorized to use the modules listed below.\n"
        ...
        await self.send_notification(
            guild,
            "⚠️ Emergency mode engaged. Our helpers are now able to use the "
    ...
    async def vaporize(self, ctx, *members: discord.Member):
        ...
        EMBED_FIELDS = [
            {"name": "Issuer", "value": f"`{ctx.author}`"},
            ...
            if is_helper:
                ...
                if em_enabled:
                    return await ctx.send(
                        "This command is only available during emergency mode. " "No such thing right now."
                    )
                else:
                    return await ctx.send("You are not authorized to issue this command.")
        ...
        for m in members:
            ...
            if rank < Rank.Rank3:
                await ctx.send(
                    "This command can only be used on Rank 3 and under. " f"`{m}` ({m.id}) is Rank {rank.value}."
                )
                return
        ...
        await self.send_notification(
            guild, f"{total} users have been vaporized.", title=EMBED_TITLE, fields=EMBED_FIELDS, jump_to=ctx.message
        )
    ...
    async def voteout(self, ctx, *, user: discord.Member):
        ...
        if not override:
            ...
            if is_staff and not has_action_perms:
                ...
                if em_enabled:
                    return await ctx.send(
                        f"You need {perm_text} permissions to use this module outside of " "emergency mode."
                    )
                else:
        ...
        if target_rank < required_rank:
            ctx.command.reset_cooldown(ctx)
            await ctx.send(
                "You cannot vote to expel that user. "
        ...
        msg = await ctx.send(
            f"A voting session to {action} user `{user}` has been initiated.\n"
        ...
        if Action(action) == Action.Ban:
            ...
            if punish_role and not self.is_role_privileged(punish_role):
                ...
                self.send_to_monitor(
                    guild,
                    "[Voteout] Failed to punish user. Is the punish role " "still present and with *no* privileges?",
                )
                await ctx.channel.send(
                    "The voting session passed but I was not able to punish the " "user due to a misconfiguration."
                )
                return
        else:
        ...
        await self.send_notification(
            guild,
            f"A user has been expelled with " f"a vote.\nVoters:\n{box(voters_list)}",
            title=EMBED_TITLE,
            fields=EMBED_FIELDS,
            jump_to=msg,
        )
        ...
        await self.create_modlog_case(
            self.bot, guild, ctx.message.created_at, action, user, guild.me, action_text, until=None, channel=None
        )
    ...
    async def silence(self, ctx: commands.Context, rank: int):
        ...
        channel = ctx.channel
        EMBED_TITLE = "🔇 • Silence"
        EMBED_FIELDS = [{"name": "Issuer", "value": f"`{ctx.author}`"}, {"name": "ID", "value": f"`{ctx.author.id}`"}]
        ...
        if not override:
            ...
            if is_staff and not has_mm_perms:
                ...
                if em_enabled:
                    return await ctx.send(
                        "You need manage messages permissions to use this " "module outside of emergency mode."
                    )
                else:
        ...
        if rank:
            ...
            await ctx.send(f"Any message from Rank {rank} and below will be deleted. " "Set 0 to disable silence mode.")
        else:
            await self.send_notification(
                guild,
                "This module has been disabled. Messages will no longer be deleted.",
                title=EMBED_TITLE,
                fields=EMBED_FIELDS,
                jump_to=ctx.message,
            )
