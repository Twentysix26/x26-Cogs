...
async def get_status_msg(guild, cog, p=None):
    ...
    if not has_core_roles_set:
        msg += f"**Configuration issue:** Core admin / mod roles are not set" + (f": see {p}set showsettings / {p}help set" if p else "") + ".\n"
    if not n_channel:
        msg += f"**Configuration issue:** Notify channel not set" + (" ({p}dset general notifychannel)" if p else "") + ".\n"
    ...
    if not n_role:
        msg += f"**Configuration issue:** Notify role not set" + (" ({p}dset general notifyrole)" if p else "") + ".\n"
