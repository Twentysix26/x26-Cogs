...
class QASelect(discord.ui.Select):
    def __init__(self, target_id: int):
        ...
        self.options = [
            ...
            SelectOption(
                value=QAInteractions.BanAndDelete24.value, label="Ban + 24h deletion", emoji="🔂"
            ),
        ]
    ...
    async def callback(self, inter: discord.Interaction):
        ...
        if await bot.is_mod(target):
            ...
            await inter.response.send_message(
                "Denied. You're trying to take action on a staff member.", ephemeral=True
            )
            return
...
class EmergencyView(discord.ui.View):
    def __init__(self, cog):
        ...
        self.cog = cog
        self.add_item(
            StopAlertButton(style=discord.ButtonStyle.danger, emoji="⚠️", label="Stop timer")
        )
