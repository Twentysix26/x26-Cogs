...
from ...core.warden.validation import (
    ALLOWED_STATEMENTS,
...
from .utils import (
    has_x_or_more_emojis,
    REMOVE_C_EMOJIS_RE,
    run_user_regex,
    make_fuzzy_suggestion,
...
MEDIA_URL_RE = re.compile(
    r"""(http)?s?:?(\/\/[^"']*\.(?:png|jpg|jpeg|gif|png|svg|mp4|gifv))""", re.I
)
URL_RE = re.compile(
    ...
    async def populate_ctx_vars(self, rule: WardenRule):
        ...
        self.state.update(
            {
                ...
                "notification_channel_id": await cog.config.guild(guild).notify_channel()
                if cog
                else 0,
            }
        )
        ...
        if self.message:
            ...
            if message.attachments:
                ...
                self.state.update(
                    {"attachment_filename": attachment.filename, "attachment_url": attachment.url}
                )
...
class WardenRule:
    ...
    async def parse(self, rule_str, cog: MixinMeta, author=None):
        ...
        if Event.Periodic in self.events:
            ...
            try:
                ...
                raise InvalidRule(
                    "The 'run-every' parameter must be between 5 minutes " "and 24 hours."
                )
            else:
                self.run_every = td
        ...
        self.action_tree = await self.parse_tree(
            rule["do"], cog=cog, author=author, events=self.events
        )
        ...
        *,
        ...
        for _dict in raw_tree:
            if not isinstance(_dict, dict):
                raise InvalidRule(
                    f"Invalid statement: `{_dict}`. Expected map. Did you forget the colon?"
                )
            ...
            try:
                if isinstance(enum, Condition):
                    ...
                    if outer_block is ConditionBlock:
                        raise InvalidRule(
                            "Conditional action blocks are not allowed inside condition blocks"
                        )
        ...
        for statement, value in tree.items():
            if runtime.debug:
                runtime.add_trace_enter(
                    stack,
                    statement.enum,
                    ignore=WDConditionalActionBlock if runtime.last_result else None,
                )

            if isinstance(statement, WDCondition):
                await self._evaluate_condition(
                    condition=statement.enum, model=value, runtime=runtime
                )
                ...
                await self.eval_tree(
                    value,
                    runtime=runtime,
                    bool_stop=block_bool_stop,
                    stack=stack,
                    outer_block=statement.enum,
                )
            elif isinstance(statement, WDConditionalActionBlock):
                can_run_true = (
                    statement.enum is ConditionalActionBlock.IfTrue and runtime.last_result is True
                )
                can_run_false = (
                    statement.enum is ConditionalActionBlock.IfFalse
                    and runtime.last_result is False
                )
    ...
    async def _evaluate_condition(
        self, condition: Condition, *, model: BaseModel, runtime: WDRuntime
    ):
        cog = runtime.cog
        ...
        async def message_matches_regex(params: models.IsStr):
            return await run_user_regex(
                rule_obj=self, cog=cog, guild=guild, regex=params.value, text=message.content
            )
        ...
        async def username_matches_regex(params: models.IsStr):
            return await run_user_regex(
                rule_obj=self, cog=cog, guild=guild, regex=params.value, text=user.name
            )
        ...
        async def nickname_matches_regex(params: models.IsStr):
            if not user.nick:
                return False
            return await run_user_regex(
                rule_obj=self, cog=cog, guild=guild, regex=params.value, text=user.nick
            )
        ...
        async def channel_is_public(params: models.IsBool):
            if parent is None:
                ...
                public = everyone not in channel.overwrites or channel.overwrites[
                    everyone
                ].read_messages in (True, None)
        ...
        async def compare(params: models.Compare):
            ...
            try:
                ...
                raise ExecutionError(
                    f"Could not compare {value1} with {value2}: they both need to be numbers!"
                )
        ...
        if result in (True, False):
            ...
            raise ExecutionError(
                f"Unexpected condition evaluation result for '{condition.value}'."
            )
    ...
    async def _do_action(self, action: Action, *, model: BaseModel, runtime: WDRuntime):
        ...
        async def notify_staff(params: models.NotifyStaff):
            ...
            if params.qa_target:
                ...
                try:
                    ...
                    raise ExecutionError(
                        f"{qa_target} is not a valid ID for a Quick Action target."
                    )
            ...
            if params.jump_to:
                ...
                try:
                    ...
                    raise ExecutionError(
                        f'{jump_to_channel_id} is not a valid channel ID for a "jump to" message.'
                    )
                if jump_to_ch:
                    try:
                        ...
                        raise ExecutionError(
                            f'{jump_to_message_id} is not a valid message ID for a "jump to" message.'
                        )
                else:
                    raise ExecutionError(
                        f'I could not find the destination channel for the "jump to" message.'
                    )
        ...
        async def remove_roles_from_user(params: models.NonEmptyList):
            ...
            if to_unassign:
                await user.remove_roles(
                    *to_unassign, reason=f"Unassigned by Warden rule '{self.name}'"
                )
        ...
        async def issue_command(params: models.IssueCommand):
            ...
            if message is None and params.destination is None:
                ...
                if params.destination is None:  # User id + command in a message context
                    ...
                    if msg_obj.channel is None:
                        raise ExecutionError(
                            f"Failed to issue command. I could not find the "
                            "notification channel."
                        )
                    if msg_obj.channel.permissions_for(issuer).view_channel is False:
                        raise ExecutionError(
                            "Failed to issue command. The issuer has no permissions "
        ...
        async def delete_last_message_sent_after(params: models.IsTimedelta):
            if runtime.last_sent_message is not None:
                cog.loop.create_task(
                    delete_message_after(runtime.last_sent_message, params.value.seconds)
                )
        ...
        async def send_message(params: models.SendMessage):
            ...
            if send_embed is False and not params.content:
                raise ExecutionError(
                    f"[Warden] ({self.name}): I have no content and " "no embed to send."
                )

            if send_embed:
                em = discord.Embed(
                    title=params.title, description=params.description, url=params.url
                )
                ...
                for field in params.fields:
                    em.add_field(
                        name=safe_sub(field.name), value=safe_sub(field.value), inline=field.inline
                    )
            ...
            mentions = discord.AllowedMentions(
                everyone=params.allow_mass_mentions,
                roles=True,
                users=True,
            ...
            if isinstance(destination, discord.Member):
                destination = (
                    ...
                    if destination.dm_channel
                    else await destination.create_dm()
                )
            ...
            if not params.edit_message_id:
                try:
                    ...
                    if is_user is False:
                        raise ExecutionError(
                            f"[Warden] ({self.name}): Failed to deliver message "
                ...
                try:
                    ...
                    await partial_msg.edit(
                        content=params.content if params.content else None,
                        embed=em,
        ...
        async def archive_and_lock_thread(params: models.IsNone):
            if isinstance(channel, discord.Thread):
                await channel.edit(
                    archived=True,
                    locked=True,
                    reason=f"Archived and locked by Warden rule '{self.name}'",
                )
        ...
        async def var_math(params: models.VarMath):
            ...
            num1, num2 = (
                safe_sub(params.operand1),
                safe_sub(params.operand2) if params.operand2 is not None else 0,
            )
        ...
        async def warnsystem_warn(params: models.WarnSystemWarn):
            ...
            if params.author:
                ...
                if ws_author is None:
                    raise ExecutionError(
                        f"I could not find the author to issue the warning ({ws_author})."
                    )
            else:
                ws_author = guild.me
