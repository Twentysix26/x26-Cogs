...
async def run_user_regex(*, rule_obj, cog, guild: discord.Guild, regex: str, text: str):
    ...
    if safety_checks_enabled:
        ...
        try:
            ...
            log.error(
                f"Warden - Unexpected error while running user defined regex with no safety checks",
                exc_info=e,
            )
            return False
...
async def rule_add_periodic_prompt(*, cog, message: discord.Message, new_rule):
    ...
    async with channel.typing():
        ...
        def confirm(r, user):
            return (
                user == message.author and str(r.emoji) == confirm_emoji and r.message.id == msg.id
            )
...
async def rule_add_overwrite_prompt(*, cog, message: discord.Message):
    ...
    msg = await channel.send(
        "There is a rule with the same name already. Do you want to "
