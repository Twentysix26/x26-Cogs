...
class BaseModel(PydanticBaseModel):
    ...
    async def _runtime_check(
        self, *, cog, author: discord.Member, action_or_cond: Union[Action, Condition]
    ):
...
class HeatKey(str):
    ...
    def validate(cls, v):
        ...
        if v.startswith("core-"):
            raise TypeError(
                "The custom heatpoint's key cannot start with 'core-': "
    ...
    def validate(cls, v):
        ...
        for char in v:
            if char not in VALID_VAR_NAME_CHARS:
                raise TypeError(
                    f"Invalid variable name. It can only contain "
...
class RolesList(NonEmptyList):
    async def _runtime_check(
        ...
        for role_id_or_name in self.value:
            ...
            if role is None:
                raise InvalidRule(
                    f"`{action_or_cond.value}`: Role `{role_id_or_name}` doesn't seem to exist."
                )
            roles.append(role)
...
class IsSlowmodeTimedelta(BaseModel):
    ...
    async def _runtime_check(
        ...
        if not author.guild_permissions.manage_channels:
            raise InvalidRule(
                f"`{action_or_cond.value}` You need `manage channels` permissions to make a rule with "
                "this action."
            )
