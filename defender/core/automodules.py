...
class AutoModules(MixinMeta, metaclass=CompositeMetaClass):  # type: ignore
    async def invite_filter(self, message):
        ...
        invite_data = (
            f"**About [discord.gg/{external_invite}](https://discord.gg/{external_invite})**\n"
        )
        ...
        try:
            ...
            if invite.guild:
                ...
                invite_data += (
                    f"I have failed to retrieve the server's data. Possibly a group DM invite?\n"
                )

        if action == Action.NoAction:
            notif_text = (
                f"I have {msg_action} a message with this content:\n{content}\n{invite_data}"
            )
        else:
    ...
    async def join_monitor_suspicious(self, member):
        ...
        description = (
            f"A user created {timestamp(member.created_at, relative=True)} just joined the server."
        )
