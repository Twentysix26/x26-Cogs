...
class AutoModules(MixinMeta, metaclass=CompositeMetaClass):  # type: ignore
    async def invite_filter(self, message):
        ...
        if await self.config.guild(guild).invite_filter_delete_message():
            ...
            try:
                ...
                self.send_to_monitor(
                    guild, "[InviteFilter] Failed to delete message: " f"no permissions in #{message.channel}"
                )
    ...
    async def detect_raider(self, message):
        ...
        if action == Action.Ban:
            ...
            await self.send_notification(
                guild,
                f"User is spamming messages ({recent} " f"messages in {minutes} minutes).",
                title=EMBED_TITLE,
                fields=EMBED_FIELDS,
                jump_to=message,
    ...
    async def join_monitor_suspicious(self, member):
        ...
        description = (
            f"A user created {timestamp(member.created_at, relative=True)} just joined the server {guild.name}."
        )
    ...
    async def comment_analysis(self, message):
        ...
        body = {"comment": {"text": message.content}, "requestedAttributes": {}, "doNotStore": True}
        ...
        await self.create_modlog_case(
            self.bot, guild, message.created_at, action.value, author, guild.me, reason, until=None, channel=None
        )
